<xsl:stylesheet
	...
	<xsl:template match="gallio:testStepRun">
		...
		<xsl:choose>
			<xsl:when test="gallio:testStep[@isTestCase='true']/gallio:metadata/gallio:entry[@key='TestKind']/gallio:value = 'Test'">
				
				<test-case>
				
					<xsl:attribute name="name"><xsl:value-of select="gallio:testStep/gallio:codeReference/@type"/>.<xsl:value-of select="gallio:testStep/@name"/></xsl:attribute>
